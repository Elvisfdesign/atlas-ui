...
> This package lives in `packages/atlas-ui/` inside the Atlas workspace
> monorepo. See the [root README](../../README.md) for how it fits together
> with `apps/atlas-intelligence`.
...
Run from the repo root (installs and links all workspaces), or from this
directory directly — both work:

```bash
npm install                     # from repo root
npm run dev --workspace=atlas-ui    # dev playground at localhost:5173
npm run storybook --workspace=atlas-ui  # component docs at localhost:6006
```

```bash
npm run build         # tsc project build + library build (dist/index.js, dist/index.d.ts, dist/styles.css)
...
Other workspace packages/apps (e.g. `apps/atlas-intelligence`) depend on
this package by name (`"atlas-ui": "*"` via the npm workspace protocol) and
import from its public surface only:

```ts
import { ThemeProvider, Button, KPICard } from 'atlas-ui';
import 'atlas-ui/styles.css'; // once, at the app root
```

`react` and `react-dom` are peer dependencies — the consuming app supplies
its own copy; Atlas UI never bundles React. Deep imports
(`atlas-ui/src/components/...`) are not supported — everything a consumer
needs is exported from the package root.
